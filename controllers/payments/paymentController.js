...
const initiateSTKPush = async (req, res) => {
  try {
    ...
    // Broadcast the initial pending transaction
...
const checkPaymentStatus = async (req, res) => {
  try {
    ...
    // Add a timeout check - if a transaction has been pending for more than 2 minutes
    // and hasn't received a callback, mark it as failed
    const twoMinutesAgo = new Date(Date.now() - 2 * 60 * 1000)
    if (transaction.status === "pending" && new Date(transaction.createdAt) < twoMinutesAgo) {
      transaction.status = "failed"
      transaction.resultDesc = "Transaction timed out"
      await transaction.save()

      // Broadcast the timeout update
      broadcastPaymentUpdate(transaction)
    }
